/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import VueRouter from 'vue-router';

import * as config from '@/shared/config/config';
import UiQueryformDetailComponent from '@/entities/ui-queryform/ui-queryform-details.vue';
import UiQueryformClass from '@/entities/ui-queryform/ui-queryform-details.component';
import UiQueryformService from '@/entities/ui-queryform/ui-queryform.service';
import router from '@/router';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();
localVue.use(VueRouter);

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
localVue.component('font-awesome-icon', {});
localVue.component('router-link', {});

describe('Component Tests', () => {
  describe('UiQueryform Management Detail Component', () => {
    let wrapper: Wrapper<UiQueryformClass>;
    let comp: UiQueryformClass;
    let uiQueryformServiceStub: SinonStubbedInstance<UiQueryformService>;

    beforeEach(() => {
      uiQueryformServiceStub = sinon.createStubInstance<UiQueryformService>(UiQueryformService);

      wrapper = shallowMount<UiQueryformClass>(UiQueryformDetailComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: { uiQueryformService: () => uiQueryformServiceStub, alertService: () => new AlertService() },
      });
      comp = wrapper.vm;
    });

    describe('OnInit', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        const foundUiQueryform = { id: 123 };
        uiQueryformServiceStub.find.resolves(foundUiQueryform);

        // WHEN
        comp.retrieveUiQueryform(123);
        await comp.$nextTick();

        // THEN
        expect(comp.uiQueryform).toBe(foundUiQueryform);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundUiQueryform = { id: 123 };
        uiQueryformServiceStub.find.resolves(foundUiQueryform);

        // WHEN
        comp.beforeRouteEnter({ params: { uiQueryformId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.uiQueryform).toBe(foundUiQueryform);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
