/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import VueRouter from 'vue-router';

import * as config from '@/shared/config/config';
import EleUnionDetailComponent from '@/entities/ele-union/ele-union-details.vue';
import EleUnionClass from '@/entities/ele-union/ele-union-details.component';
import EleUnionService from '@/entities/ele-union/ele-union.service';
import router from '@/router';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();
localVue.use(VueRouter);

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
localVue.component('font-awesome-icon', {});
localVue.component('router-link', {});

describe('Component Tests', () => {
  describe('EleUnion Management Detail Component', () => {
    let wrapper: Wrapper<EleUnionClass>;
    let comp: EleUnionClass;
    let eleUnionServiceStub: SinonStubbedInstance<EleUnionService>;

    beforeEach(() => {
      eleUnionServiceStub = sinon.createStubInstance<EleUnionService>(EleUnionService);

      wrapper = shallowMount<EleUnionClass>(EleUnionDetailComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: { eleUnionService: () => eleUnionServiceStub, alertService: () => new AlertService() },
      });
      comp = wrapper.vm;
    });

    describe('OnInit', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        const foundEleUnion = { id: 123 };
        eleUnionServiceStub.find.resolves(foundEleUnion);

        // WHEN
        comp.retrieveEleUnion(123);
        await comp.$nextTick();

        // THEN
        expect(comp.eleUnion).toBe(foundEleUnion);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundEleUnion = { id: 123 };
        eleUnionServiceStub.find.resolves(foundEleUnion);

        // WHEN
        comp.beforeRouteEnter({ params: { eleUnionId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.eleUnion).toBe(foundEleUnion);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
