/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import VueRouter from 'vue-router';

import * as config from '@/shared/config/config';
import DataPermissionDetailComponent from '@/entities/data-permission/data-permission-details.vue';
import DataPermissionClass from '@/entities/data-permission/data-permission-details.component';
import DataPermissionService from '@/entities/data-permission/data-permission.service';
import router from '@/router';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();
localVue.use(VueRouter);

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
localVue.component('font-awesome-icon', {});
localVue.component('router-link', {});

describe('Component Tests', () => {
  describe('DataPermission Management Detail Component', () => {
    let wrapper: Wrapper<DataPermissionClass>;
    let comp: DataPermissionClass;
    let dataPermissionServiceStub: SinonStubbedInstance<DataPermissionService>;

    beforeEach(() => {
      dataPermissionServiceStub = sinon.createStubInstance<DataPermissionService>(DataPermissionService);

      wrapper = shallowMount<DataPermissionClass>(DataPermissionDetailComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: { dataPermissionService: () => dataPermissionServiceStub, alertService: () => new AlertService() },
      });
      comp = wrapper.vm;
    });

    describe('OnInit', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        const foundDataPermission = { id: 123 };
        dataPermissionServiceStub.find.resolves(foundDataPermission);

        // WHEN
        comp.retrieveDataPermission(123);
        await comp.$nextTick();

        // THEN
        expect(comp.dataPermission).toBe(foundDataPermission);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundDataPermission = { id: 123 };
        dataPermissionServiceStub.find.resolves(foundDataPermission);

        // WHEN
        comp.beforeRouteEnter({ params: { dataPermissionId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.dataPermission).toBe(foundDataPermission);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
