...
public class ActivityService {
    ...
    public ProcessResult doAction(final String bpmnType, final Action action, final List<Map> datas, final String userId) {
        try {
            ...
            processResult.setResult(resultList);
            for (Map data : datas) {
                param.put("businessKey", data.get("id"));
                ProcessResult tmpProcessResult = new ProcessResult();
                if (action.isCancel()) {
                    //                doCancel();
                    ...
                    tmpProcessResult = doCreate(param);
                } else {
                    String variable = action.getVariable();
                    tmpProcessResult = doAudit(param);
                }

                resultList.addAll((List<Map>) tmpProcessResult.getResult());
            }
            logger.error(
                "执行动作" +
                (action.isCancel() ? "Cancel" : "") +
    ...
    private ProcessResult doCreate(Map<String, Object> param) {
        ...
        ProcessInstance processInstance = runtimeService.startProcessInstanceById(processDefinitionId, businessKey, param);
        logger.info(
