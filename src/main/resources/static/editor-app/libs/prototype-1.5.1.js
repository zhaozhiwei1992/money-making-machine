...
Ajax.Request.prototype = Object.extend(new Ajax.Base(), {
  ...
  request: function (url) {
    ...
    if ((params = Hash.toQueryString(params))) {
      ...
      if (this.method == 'get' || this.options.postBody) this.url += (this.url.include('?') ? '&' : '?') + params;
  ...
  setRequestHeaders: function () {
    ...
    if (typeof this.options.requestHeaders == 'object') {
      ...
      if (typeof extras.push == 'function') for (var i = 0, length = extras.length; i < length; i += 2) headers[extras[i]] = extras[i + 1];
      else
  ...
  respondToReadyState: function (readyState) {
    ...
    if (state == 'Complete') {
      ...
      if (contentType && contentType.strip().match(/^(text|application)\/(x-)?(java|ecma)script(;.*)?$/i)) this.evalResponse();
    }
...
Object.extend(Selector, {
  ...
  handlers: {
    ...
    tagName: function (nodes, root, tagName, combinator) {
      ...
      if (nodes) {
        ...
        for (var i = 0, node; (node = nodes[i]); i++) if (node.tagName.toUpperCase() == tagName) results.push(node);
        return results;
  ...
  pseudos: {
    ...
    not: function (nodes, selector, root) {
      ...
      h.mark(exclusions);
      for (var i = 0, results = [], node; (node = nodes[i]); i++) if (!node._counted) results.push(node);
      h.unmark(exclusions);
      return results;
    },
  ...
  matchElements: function (elements, expression) {
    var matches = new Selector(expression).findElements(),
      h = Selector.handlers;
    h.mark(matches);
    for (var i = 0, results = [], element; (element = elements[i]); i++) if (element._counted) results.push(element);
    h.unmark(matches);
    return results;
  },
...
Abstract.TimedObserver.prototype = {
  ...
  onTimerEvent: function () {
    ...
    var changed =
      'string' == typeof this.lastValue && 'string' == typeof value ? this.lastValue != value : String(this.lastValue) != String(value);
    if (changed) {
