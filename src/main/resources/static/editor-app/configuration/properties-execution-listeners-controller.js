...
var KisBpmExecutionListenersCtrl = [
  '$scope',
  '$modal',
  '$timeout',
  '$translate',
...
var KisBpmExecutionListenersPopupCtrl = [
  '$scope',
  '$q',
  '$translate',
  function ($scope, $q, $translate) {
    ...
    $scope.addNewListener = function () {
      $scope.executionListeners.push({ event: 'start', implementation: '', className: '', expression: '', delegateExpression: '' });
    };
    ...
    $scope.moveListenerDown = function () {
      if ($scope.selectedListeners.length > 0) {
        var index = $scope.executionListeners.indexOf($scope.selectedListeners[0]);
        if (index != $scope.executionListeners.length - 1) {
