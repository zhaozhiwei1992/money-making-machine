...
var KisBpmEventListenersPopupCtrl = [
  '$scope',
  '$q',
  '$translate',
  function ($scope, $q, $translate) {
    ...
    $scope.addNewListener = function () {
      $scope.eventListeners.push({ event: '', implementation: '', className: '', delegateExpression: '', retrowEvent: false });
    };
    ...
    $scope.moveListenerDown = function () {
      if ($scope.selectedListeners.length > 0) {
        var index = $scope.eventListeners.indexOf($scope.selectedListeners[0]);
        if (index != $scope.eventListeners.length - 1) {
