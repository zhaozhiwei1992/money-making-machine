...
angular.module('activitiModeler').controller('StencilController', [
  '$rootScope',
  '$scope',
  '$http',
  '$modal',
  ...
  function ($rootScope, $scope, $http, $modal, $timeout) {
    ...
    $scope.editorFactory.promise.then(function () {
      ...
      /*
             StencilSet items
             */
      $http({ method: 'GET', url: KISBPM.URL.getStencilSet() })
        ...
        KISBPM.eventBus.addListener(KISBPM.eventBus.EVENT_TYPE_PROPERTY_VALUE_CHANGED, function (event) {
          if (event.property && event.property.key) {
            ...
            event.property.noValue =
              event.property.value === undefined || event.property.value === null || event.property.value.length == 0;
          }
        });
      ...
      if ($scope.dragCanContain) {
        ...
        if ($scope.quickMenu) {
          ...
          if (shapes && shapes.length == 1) {
            ...
            if (
              containedStencil.idWithoutNs() !== 'SequenceFlow' &&
          ...
          var canAttach = false;
          if (
            containedStencil.idWithoutNs() === 'BoundaryErrorEvent' ||
