window.onload = function () {
  ...
  var font = { font: '11px Arial', 'font-style': 'italic', opacity: 1, fill: LABEL_COLOR, stroke: LABEL_COLOR, 'stroke-width': 0.3 };
  ...
  AttributedStringIterator.prototype = {
    getEndIndex: function (pos) {
      if (typeof pos == 'undefined') return this.endIndex;

      var string = this.text.substr(pos, this.endIndex - pos);

      var posEndOfLine = string.search(/[\n]/);
      if (posEndOfLine == -1) return this.endIndex;
      else return pos + posEndOfLine;
    },
    ...
    getLineBreakIndex: function (pos, wrappingWidth) {
      ...
      if (charNum == -1) {
        ...
        if (newLineIndex < charNum) {
          console.log(
            'newLineIndex <= charNum, newLineIndex:' + newLineIndex + ', charNum:' + charNum,
            '"' + this.text.text.substr(newLineIndex + 1).replace(/\n/g, '↵') + '"'
          );
          //console.groupEnd();
  ...
  function drawMultilineText(text, x, y, boxWidth, boxHeight, options) {
    ...
    if (boxHeight) var height = boxHeight - 2 * TEXT_PADDING;
    ...
    if (options) measurer.rafaelTextObject.attr({ 'text-anchor': options['text-anchor'] });

    var bb = measurer.rafaelTextObject.getBBox();
    //measurer.rafaelTextObject.attr({"x": x + boxWidth/2});
    if (options['vertical-align'] == 'top') measurer.rafaelTextObject.attr({ y: y + bb.height / 2 + TEXT_PADDING });
    else measurer.rafaelTextObject.attr({ y: y + height / 2 });
    //var bb = measurer.rafaelTextObject.getBBox();

    if (measurer.rafaelTextObject.attr('text-anchor') == 'middle') measurer.rafaelTextObject.attr('x', x + boxWidth / 2 + TEXT_PADDING / 2);
    else if (measurer.rafaelTextObject.attr('text-anchor') == 'end') measurer.rafaelTextObject.attr('x', x + boxWidth + TEXT_PADDING / 2);
    else measurer.rafaelTextObject.attr('x', x + boxWidth / 2 - bb.width / 2 + TEXT_PADDING / 2);
    ...
    /*
				...
				*/
    var boxStyle = { stroke: Color.LightSteelBlue2, 'stroke-width': 1.0, 'stroke-dasharray': '- ' };
    var rect = paper.rect(x + 0.5, y + 0.5, boxWidth, boxHeight).attr(boxStyle);
  }
